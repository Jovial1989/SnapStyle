...
// src/stylistPrompt.js
var PROMPT_VERSION = "stylist-v2-hotspots";
var SYSTEM_PROMPT = `You are Snapstyle \u2014 an expert personal stylist reviewing a photo of someone's outfit and fit.
  ...
  properties: {
    ...
    overall: {
      ...
      properties: {
        summary: { type: "STRING" },
        score: { type: "INTEGER" }
        // integer 1–10 ONLY (not a percentage)
      },
    ...
    hotspots: {
      ...
      items: {
        ...
        properties: {
          ...
          area: { type: "STRING" },
          // shoulders | chest | waist | hips | knee | ankle | shoes | overall
          ...
          visual_suggestions: {
            ...
            items: {
              ...
              properties: {
                prompt: { type: "STRING" },
                // image-gen prompt (brand-agnostic) — WE render it
                caption: { type: "STRING" },
                // ≤4 words
...
app.post("/analyze", async (c) => {
  ...
  if (!res.ok) return c.json({ error: "analysis_failed", status: res.status, detail: (await res.text()).slice(0, 400) }, 502);
  const json = await res.json();
  const text = json?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!text) return c.json({ error: "analysis_failed", reason: "no_text", block: json?.promptFeedback ?? null }, 502);
